...
- 🎯 Entry points to execute your program using `python -m <ruggle_fastapi>` or `$ ruggle_fastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/EpicJM/ruggle_fastapi/branch/main/graph/badge.svg?token=ruggle_fastapi_token_here)](https://codecov.io/gh/EpicJM/ruggle_fastapi)
[![CI](https://github.com/EpicJM/ruggle_fastapi/actions/workflows/main.yml/badge.svg)](https://github.com/EpicJM/ruggle_fastapi/actions/workflows/main.yml)

Awesome ruggle_fastapi created by EpicJM

## Install

from source
```bash
git clone https://github.com/EpicJM/ruggle_fastapi ruggle_fastapi
cd ruggle_fastapi
make install
```
...
```bash
pip install ruggle_fastapi
```

## Executing

```bash
$ ruggle_fastapi run --port 8080
```

or

```bash
python -m ruggle_fastapi run --port 8080
```

or

```bash
$ uvicorn ruggle_fastapi:app
```
...
```bash
❯ ruggle_fastapi --help
Usage: ruggle_fastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ruggle_fastapi create-user --help
Usage: ruggle_fastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ruggle_fastapi create-user admin admin --superuser
```
...
Run with `ruggle_fastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ruggle_fastapi create-user`.
...
-----------------------------------------------------
ruggle_fastapi/__init__.py              4      0   100%
ruggle_fastapi/app.py                  16      1    94%
ruggle_fastapi/cli.py                  21      0   100%
ruggle_fastapi/config.py                5      0   100%
ruggle_fastapi/db.py                   10      0   100%
ruggle_fastapi/models/__init__.py       0      0   100%
ruggle_fastapi/models/content.py       47      1    98%
ruggle_fastapi/routes/__init__.py      11      0   100%
ruggle_fastapi/routes/content.py       52     25    52%
ruggle_fastapi/routes/security.py      15      1    93%
ruggle_fastapi/routes/user.py          52     26    50%
ruggle_fastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from ruggle_fastapi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ruggle_fastapi/default.toml.

### As environment variables
```bash
export ruggle_fastapi_KEY=value
export ruggle_fastapi_KEY="@int 42"
export ruggle_fastapi_KEY="@jinja {{ this.db.uri }}"
export ruggle_fastapi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
ruggle_fastapi_ENV=production ruggle_fastapi run
```
